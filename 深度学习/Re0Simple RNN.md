...
* 我们在这里打印每个数据集的样本数量，并随机查看一个训练样本的内容，以人工验证我们的数据加载和预处理流程是否符合预期。这是保证后续工作建立在正确基础上的关键一步。
  ...
  **`src/data_loader.py`**
  ...
  5.  **构建词汇表 (`build_vocab`)**:
      1.  **构建词汇表 (`build_vocab`)：一个两阶段策略的准备工作**
          *   `TEXT.build_vocab(...)`: 这是数据准备阶段最核心的一步，我们可以理解为“**准备阶段**”。
          *   `max_size=25_000`: 限制词汇表大小，过滤低频噪音词。
          *   `vectors="glove.6B.100d"`: **这是关键**。这行代码并**不是**让数据迭代器直接使用词向量。它的作用是命令`torchtext`：
              1.  自动下载预训练好的GloVe词向量。
              2.  在构建我们自己的词汇表时，检查每个词是否也存在于GloVe的词汇表中。
              3.  如果存在，就将该词的GloVe向量提取出来，存储在一个名为 `TEXT.vocab.vectors` 的**临时“仓库”张量**中。这个“仓库”将在模型初始化时被使用。
          *   `unk_init=torch.Tensor.normal_`: 对于我们词汇表中存在，但在GloVe中找不到的词（OOV, Out-of-Vocabulary），用一个随机向量来初始化它们在“仓库”中的位置。
  
      2.  **创建`BucketIterator`：高效的索引“邮递员”**
          *   `BucketIterator` 的职责非常纯粹：它高效地将长度相近的句子打包成批次，然后将这些句子对应的**数字索引张量**发送出去。
          *   **重要**：迭代器本身并**不处理或发送**词向量。它只是一个高效的**索引**批处理工具。真正的向量化转换发生在模型内部。

      3.  **返回**:
          *   函数最终返回了`TEXT`字段（**它携带着包含预训练向量的`.vocab.vectors`仓库**）、`LABEL`字段以及三个数据迭代器，为下一步在`train.py`中初始化模型做好了万全的准备。
  6.  **创建`BucketIterator`**:
...
*   `return self.fc(hidden)`:
    * 将处理好的最终隐藏状态送入全连接层，得到形状为`[batch_size, output_dim]`的原始预测分数（logits）。
      ...
      ### **第一部分：`forward`方法处理的到底是什么？—— 小批次(Mini-Batch)的真相**
    
      首先，我们必须澄清，`forward`方法**既不处理单个词，也不处理单个句子**。
    
      它处理的是一个**“小批次（Mini-Batch）”**，也就是**一批句子**。
    
      想象一下，我们的`DataLoader`是一个发牌员，它不会一张一张地给你牌，而是一次给你一手牌（比如64张）。在我们的项目中：
    
      *   **一个“小批次”就是一组句子**（例如，64个电影评论）。
      *   为了让计算机能处理，这些句子都被**填充（padding）**到了这个批次中最长句子的长度。
      *   每个词都已经被转换成了数字索引。
    
      所以，当`forward(self, text)`被调用时，传入的`text`参数是一个**单一的、巨大的数字矩阵（张量）**。
    
      它的形状是 `[batch_size, sequence_length]`。
    
      *   `batch_size`: 批次中的句子数量（例如 64）。
      *   `sequence_length`: 这个批次中最长句子的单词数（例如 500）。
    
      **第一步：进入嵌入层**
    
      代码 `embedded = self.dropout(self.embedding(text))` 做的事情是：
      将这个 `[64, 500]` 的数字索引矩阵，变成了 `[64, 500, 100]` 的浮点数张量。
      *   `100` 是我们设置的`embedding_dim`。
      *   现在，我们有64个句子，每个句子由500个100维的向量表示。
    
      **到目前为止，我们还没有做任何“循环”或“时间”相关的处理。我们只是把输入的“一手牌”从数字索引变成了词向量。**
    
      ---
    
      ### **第二部分：进入`self.rnn(embedded)` —— 时间、层次与方向的交响曲**
    
      这是最核心的一步。当形状为 `[64, 500, 100]` 的`embedded`张量进入RNN层时，PyTorch会在内部同时处理三个维度：**批次、时间和层次**。
    
      让我们用一个具体的例子来解释：假设我们的模型设置是：
      *   `n_layers = 2` (一个两层的RNN)
      *   `bidirectional = True` (双向)
    
      您可以把这个RNN层想象成一个**有两层楼的工厂**，每一层楼都有**两条并行的生产线**。
    
      #### **工厂的运作流程：**
    
      1.  **原材料入场**:
          *   我们的`embedded`张量 `[64, 500, 100]` 就是64份独立的原材料。
          *   `sequence_length=500` 这个维度，就是**“时间”维度**。RNN将沿着这个维度进行500步的处理。
    
      2.  **一楼（Layer 1）开始工作**:
          *   **生产线A（前向）**: 从时间步1开始，处理到时间步500。
              *   在时间步1，它接收第1个词的词向量，产出一个隐藏状态。
              *   在时间步2，它接收第2个词的词向量 **和** 上一步的隐藏状态，产出新的隐藏状态。
              *   ...这个过程持续500步。
          *   **生产线B（后向）**: **与生产线A同时进行！** 它从时间步500开始，**反向**处理到时间步1。
              *   在时间步500，它接收第500个词的词向量，产出一个隐藏状态。
              *   在时间步499，它接收第499个词的词向量 **和** 它自己的上一步（即500步时）的隐藏状态，产出新的隐藏状态。
              *   ...这个过程也持续500步。
    
      3.  **一楼的产出，送往二楼**:
          *   在一楼的**每一个时间步**，生产线A和生产线B都会各自产出一个隐藏状态。PyTorch会将这两个隐藏状态**拼接**起来。
          *   这个拼接后的结果，将作为**二楼（Layer 2）在那一个时间步的输入**。
          *   所以，二楼看到的不再是原始的词向量，而是一楼处理和抽象过的信息。
    
      4.  **二楼（Layer 2）重复同样的工作**:
          *   二楼也有两条自己独立的生产线（前向C和后向D）。
          *   它们接收来自一楼的拼接后的输入，并以完全相同的前向和后向方式进行处理。
    
      #### **最终的产成品：`outputs` 和 `hidden`**
    
      当这500个时间步全部处理完毕后，这个工厂会交给你两样东西：
    
      1.  **`outputs`**:
          *   它是在**最后一个时间步**，从**最顶层（二楼）** 的两条生产线（C和D）产出的**每一个时间步**的拼接结果。
          *   它的形状是 `[batch_size, seq_len, hidden_dim * 2]`，即 `[64, 500, 512]`。
          *   它包含了每个词在融合了**整个句子双向上下文信息后**的最高级表示。在我们的分类任务中暂时用不到它，但在问答、序列标注等任务中至关重要。
    
      2.  **`hidden`**:
          *   这是**所有生产线**在**完成它们各自的全部流程后**的**最终状态**的**集合**。可以理解为工厂停工时，每条生产线最后那个位置的状态快照。
          *   它的形状是 `[n_layers * 2, batch_size, hidden_dim]`，即 `[4, 64, 256]`。
    
      ---
    
      ### **第三部分：彻底解码`hidden`张量 —— 代码与逻辑的最终对应**
    
      现在，我们来仔细剖析这个形状为 `[4, 64, 256]` 的`hidden`张量。**这正是您问题的核心**。
    
      PyTorch按照一个固定的顺序来堆叠这4条生产线的最终状态：
    
      *   `hidden[0, :, :]`: **一楼（Layer 1），前向生产线A** 的最终状态。
      *   `hidden[1, :, :]`: **一楼（Layer 1），后向生产线B** 的最终状态。
      *   `hidden[2, :, :]`: **二楼（Layer 2），前向生产线C** 的最终状态。
      *   `hidden[3, :, :]`: **二楼（Layer 2），后向生产线D** 的最终状态。
      ...
      **现在，请看您的代码：**
      ...
      在Python中，`-1`索引代表最后一个元素，`-2`代表倒数第二个。
    
      *   `hidden[-1, :, :]`  就是 `hidden[3, :, :]`。根据我们的对应表，它正好是 **最顶层（二楼）、后向** 的最终状态。
      *   `hidden[-2, :, :]`  就是 `hidden[2, :, :]`。根据我们的对应表，它正好是 **最顶层（二楼）、前向** 的最终状态。
    
      **所以，这行代码的含义无比清晰了：**
    
      **“我不需要底层（一楼）的最终总结，因为它们的信息已经被顶层（二楼）学习和吸收了。我只需要从最高抽象层次（二楼），分别拿到它对整个句子的‘前向总结’和‘后向总结’，然后把这两个最精华的总结拼接在一起，作为代表整个句子的最终向量，送去做分类。”**
